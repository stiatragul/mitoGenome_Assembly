...
Instead of running these scripts on a local machine you may want to run them on a server with more memory elsewhere. If you can install and run [RStudio Server](https://www.rstudio.com/products/rstudio/download-server/) on your remote machine, you can log-in and run this workflow using the same file structure and instructions as above.  

I've included the MIRA linux version in this repository but you might find you need to download and install it yourself. Make sure to establish the directory structure as outlined on page 3. Once you open an RStudio Server window, you'll need to source the functions we'll need.
```{r eval=FALSE}
source("/PATH_TO_CODE/mtGenome_Assembly_Nick.R")
```
Then set your working directory to the project-specific directory (I've called this directory 'Frogs'). This holds a subdirectory for each sample, and your reference mitogenome.
```{r eval=FALSE}
setwd("/PATH_TO_DIR/mtGenomes/Frogs")
